...
def batch_stock_list(request):
    ...
    active_count = all_batches.filter(
        Q(expiry_date__gt=near_limit) | Q(expiry_date__isnull=True)
    ).count()
...
def adjust_batch_stock(request):
    ...
    if request.method == "POST":
        ...
        #  Prevent DB Overflow Error and enforce business limit
        if quantity > 10000:
            messages.error(request, "Quantity is too large. Maximum allowed adjustment is 10,000.")
            return redirect("adjust_batch_stock")
...
def customer_export_excel(request):
    ...
    # Only current company customers
    ...
    # Export only existing customers
    for c in customers:
        ws.append([
            c.id,
            c.contact_name or "",
